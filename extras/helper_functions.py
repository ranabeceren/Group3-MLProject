...
def train_step(model: torch.nn.Module,
    ...
    """Preforms a training loop step on the model trying to learn on data_loader."""
    ...
    for batch, (X,y) in enumerate(data_loader):
        ...
        train_acc += accuracy_fn(y_true=y,
                                 y_pred=y_pred.argmax(dim=1)) # from logits -> to labels
        
        # Other training steps (per batch)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()

    # Average train_loss & train_acc (per batch)
